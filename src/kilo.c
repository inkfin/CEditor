...
{
    int cx, cy;
    /* the render x loc, spatial characters that take more than 1 mono-space, like tabs */
    int rx;
    int rowoff;
    int coloff;
    int screenrows;
...
void editorDrawRows(struct abuf *ab)
{
    ...
    for (y = 0; y < E.screenrows; y++)
    {
        ...
        // outside write buffer
        {
            if (E.numrows == 0 && // only draw hello screen when open empty files
                y == E.screenrows / 3)
            // Draw hello screen
            {
                char welcome[80];
